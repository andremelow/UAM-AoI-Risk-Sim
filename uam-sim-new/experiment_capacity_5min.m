function experiment_capacity_5min(varargin)
%% EXPERIMENT_CAPACITY_5MIN  Sweep K × C × routing × policy — steady 5 min.
%
%  Modelo capacity_drain:
%    Fase fill:   drones entram com spacing = flightTime/C_route até C simultâneos.
%    Fase steady: C drones ativos por steady_duration = 300 s (5 min).
...
%
%  Total: 6K × 6C × 4 routing × 6 policies = 864 runs.
%  Corredor: 2 km, dois lanes paralelos (North ±50 m), speed = 5 m/s.
%
%  Output: csv_export/capacity_5min/K{K}/C{CC}/{routing_label}/
%
%  Usage:
%    experiment_capacity_5min()
%    experiment_capacity_5min('--smoke-only')
%    experiment_capacity_5min('skip-smoke')
%    experiment_capacity_5min('--yes')
%    experiment_capacity_5min('--headless')
%    experiment_capacity_5min('--C', '2,4,8')    % subset de capacidades
%    experiment_capacity_5min('--K', '1,2,4')    % subset de K
...
T_STEADY = 300;  % 5 minutos em regime estável

% ── Output root ───────────────────────────────────────────────────────
SIM_ROOT = fileparts(mfilename('fullpath'));
OUT_ROOT = fullfile(SIM_ROOT, 'csv_export', 'capacity_5min');
system(sprintf('mkdir -p "%s"', OUT_ROOT));

% ── Logfile ───────────────────────────────────────────────────────────
ts      = datestr(now, 'yyyymmdd_HHMMSS'); %#ok<TNOW1,DATST>
logFile = fullfile(OUT_ROOT, sprintf('experiment_capacity_5min_%s.log', ts));
diary(logFile); diary on;

fprintf('\n');
fprintf('═══════════════════════════════════════════════════════════════\n');
fprintf('  EXPERIMENT_CAPACITY_5MIN — K × C × routing × policy\n');
fprintf('  Corredor: 2 km East, 2 lanes paralelos (N±50 m)  speed=5 m/s\n');
fprintf('  Modelo: capacity_drain  steady=%ds (5 min)\n', T_STEADY);
...
if ~skip_smoke || smoke_only
    fprintf('\n──────────────────────────────────────────────────────────\n');
    fprintf('  SMOKE TEST (corredor 200m, C=2, K=1, T=30s, 2 políticas × 2 cenários)\n');
    fprintf('──────────────────────────────────────────────────────────\n\n');
    smoke_t0 = tic;
...
% ── Sweep ─────────────────────────────────────────────────────────────
fprintf('\n──────────────────────────────────────────────────────────\n');
fprintf('  CAPACITY_5MIN SWEEP  (%d runs, %d workers)\n', n_combos, nWorkers);
fprintf('──────────────────────────────────────────────────────────\n\n');
...
fprintf('\n[CAPACITY_5MIN] Sweep concluído em %.2f h\n', toc(sw_t0)/3600);
...
fprintf('[CAPACITY_5MIN] Aggregated → %s\n', all_csv);

fprintf('\n═══════════════════════════════════════════════════════════════\n');
fprintf('  EXPERIMENT_CAPACITY_5MIN COMPLETED\n');
fprintf('  Finished: %s\n', datestr(now)); %#ok<TNOW1,DATST>
...
fclose(fid);
fprintf('[CAPACITY_5MIN] Summary → %s\n', filepath);
end


%% ═══════════════════════════════════════════════════════════════════
%%  Smoke test — corredor 200m, C=2, K=1, T=30s, 2 políticas × 2 cenários
%% ═══════════════════════════════════════════════════════════════════
