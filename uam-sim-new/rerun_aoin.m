%% RERUN_AOIN  Re-runs only aoi-normalized across the full (K,C) grid.
%  Use after fixing z_u startup floor in step_risk.m to regenerate affected CSVs.
%
%  Usage (headless):
%    matlab -nodisplay -r "cd /mnt/stg/wrk/interlagos/simulator/v3/uam-sim-new; rerun_aoin; exit"
...
logFile = fullfile(OUT_ROOT, sprintf('rerun_aoin_%s.log', ts));
diary(logFile); diary on;

fprintf('\n');
fprintf('═══════════════════════════════════════════════════════════════\n');
fprintf('  RERUN aoi-normalized  (fix: aoi-normalized policy)\n');
fprintf('  K: %s\n', num2str(K_values));
...
T_old = T_old(T_old.policy ~= "aoi-normalized", :);
...
fprintf('[RERUN_AOIN] Global summary updated → %s  (%d rows)\n', new_all, height(T_merged));
