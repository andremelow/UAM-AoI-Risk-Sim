%% RERUN_MWDRIFT  Re-runs only max-weight-drift across the full (K,C) grid.
%  Use after changing the W_s formula in policy_max_weight_drift.m.
%
%  Usage (headless):
%    matlab -nodisplay -r "cd /mnt/stg/wrk/interlagos/simulator/v3/uam-sim-new; rerun_mwdrift; exit"
...
logFile = fullfile(OUT_ROOT, sprintf('rerun_mwdrift_%s.log', ts));
diary(logFile); diary on;

fprintf('\n');
fprintf('═══════════════════════════════════════════════════════════════\n');
fprintf('  RERUN max-weight-drift  (fix: W_s = (h+1)^2 baseline)\n');
fprintf('  K: %s\n', num2str(K_values));
...
fprintf('\n[RERUN_MWDRIFT] Concluído em %.2f h\n', toc(sw_t0)/3600);

% Rebuild global summary merging with existing non-mwdrift rows
...
T_old = T_old(T_old.policy ~= "max-weight-drift", :);
...
fprintf('[RERUN_MWDRIFT] Global summary updated → %s  (%d rows)\n', new_all, height(T_merged));
