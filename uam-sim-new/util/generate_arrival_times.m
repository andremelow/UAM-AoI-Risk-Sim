...
switch model
    ...
    % ------------------------------------------------------------------
    case 'capacity_drain'
        % Fill corridor to C, maintain C for steady_duration seconds, then drain.
        %
        % cfg.corridorCapacity — C, concurrent drones during steady state
        % cfg.steady_duration  — seconds at full capacity (default 300)
        %
        % Timeline:
        %   [0, (C-1)*spacing]         fill phase:   drones 0..C-1 enter
        %   [(C-1)*spacing, +T_steady] steady phase: C drones always active
        %   [last_entry + spacing, +]  drain phase:  no new entries
        %
        % N_total = C + floor(T_steady / spacing)
        % where spacing = flightTime / C.

        C        = get_field(cfg, 'corridorCapacity', 1);
        T_steady = get_field(cfg, 'steady_duration',  300);
        spacing  = cfg.flightTime / max(C, 1);
        N_exp    = C + floor(T_steady / spacing);
        if N ~= N_exp
            warning('generate_arrival_times:capacity_drain', ...
                ['capacity_drain expects numDrones=%d but got %d. ' ...
                 'Use build_capacity_run_cfg to set numDrones correctly.'], N_exp, N);
        end
        t = cfg.minStartDelay + (0:N-1)' * spacing;

        t_full   = cfg.minStartDelay + (C-1) * spacing;
        t_drain  = cfg.minStartDelay + (N-1) * spacing + spacing;
        arrivalMeta.description      = sprintf('Capacity-drain C=%d T_steady=%.0fs', C, T_steady);
        arrivalMeta.corridorCapacity = C;
        arrivalMeta.steady_duration  = T_steady;
        arrivalMeta.spacing_s        = spacing;
        arrivalMeta.t_full_s         = t_full;
        arrivalMeta.t_drain_s        = t_drain;
        arrivalMeta.lambda_eff       = C / cfg.flightTime;

    % ------------------------------------------------------------------
    case 'relay'
        % Relay at corridor capacity C: consecutive drones separated by
        % flightTime/C seconds so exactly C drones are active at all times.
        %
        % cfg.corridorCapacity  — C, max concurrent drones in corridor
        % cfg.relay_laps        — number of full corridor traversals (default 5)
        %                         numDrones must equal C × relay_laps
        %
        % Arrival rate: lambda = C / flightTime [drones/s]
        % At steady state: active set size ≡ C.

        C       = get_field(cfg, 'corridorCapacity', 1);
        R       = get_field(cfg, 'relay_laps',        5);
        N_exp   = C * R;
        if N ~= N_exp
            warning('generate_arrival_times:relay', ...
                ['relay expects numDrones=%d (C=%d × laps=%d) but got numDrones=%d. ' ...
                 'Adjusting spacing to fit %d drones.'], N_exp, C, R, N, N);
        end
        spacing = cfg.flightTime / max(C, 1);
        t = cfg.minStartDelay + (0:N-1)' * spacing;

        arrivalMeta.description      = sprintf('Relay capacity C=%d (%d laps)', C, R);
        arrivalMeta.corridorCapacity = C;
        arrivalMeta.relay_laps       = R;
        arrivalMeta.spacing_s        = spacing;
        arrivalMeta.lambda_eff       = C / cfg.flightTime;
