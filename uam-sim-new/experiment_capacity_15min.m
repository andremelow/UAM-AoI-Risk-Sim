function experiment_capacity_15min(varargin)
%% EXPERIMENT_CAPACITY_15MIN  Sweep K × C × routing × policy — steady 15 min.
%
...
%
%  Output: csv_export/capacity_15min/K{K}/C{CC}/{routing_label}/
%
%  Usage:
%    experiment_capacity_15min()
%    experiment_capacity_15min('--smoke-only')
%    experiment_capacity_15min('skip-smoke')
%    experiment_capacity_15min('--yes')
%    experiment_capacity_15min('--headless')
%    experiment_capacity_15min('--C', '2,4,8')    % subset de capacidades
%    experiment_capacity_15min('--K', '1,2,4')    % subset de K
...
OUT_ROOT = fullfile(SIM_ROOT, 'csv_export', 'capacity_15min');
system(sprintf('mkdir -p "%s"', OUT_ROOT));

% ── Logfile ───────────────────────────────────────────────────────────
ts      = datestr(now, 'yyyymmdd_HHMMSS'); %#ok<TNOW1,DATST>
logFile = fullfile(OUT_ROOT, sprintf('experiment_capacity_15min_%s.log', ts));
diary(logFile); diary on;

fprintf('\n');
fprintf('═══════════════════════════════════════════════════════════════\n');
fprintf('  EXPERIMENT_CAPACITY_15MIN — K × C × routing × policy\n');
...
if ~skip_smoke || smoke_only
    fprintf('\n──────────────────────────────────────────────────────────\n');
    fprintf('  SMOKE TEST (corredor 200m, C=2, K=1, T=60s, 2 políticas × 2 cenários)\n');
    fprintf('──────────────────────────────────────────────────────────\n\n');
    smoke_t0 = tic;
...
% ── Sweep ─────────────────────────────────────────────────────────────
fprintf('\n──────────────────────────────────────────────────────────\n');
fprintf('  CAPACITY_15MIN SWEEP  (%d runs, %d workers)\n', n_combos, nWorkers);
fprintf('──────────────────────────────────────────────────────────\n\n');
...
fprintf('\n[CAPACITY_15MIN] Sweep concluído em %.2f h\n', toc(sw_t0)/3600);
...
fprintf('[CAPACITY_15MIN] Aggregated → %s\n', all_csv);

fprintf('\n═══════════════════════════════════════════════════════════════\n');
fprintf('  EXPERIMENT_CAPACITY_15MIN COMPLETED\n');
fprintf('  Finished: %s\n', datestr(now)); %#ok<TNOW1,DATST>
...
fclose(fid);
fprintf('[CAPACITY_15MIN] Summary → %s\n', filepath);
end


%% ═══════════════════════════════════════════════════════════════════
%%  Smoke test — corredor 200m, C=2, K=1, T=60s, 2 políticas × 2 cenários
%% ═══════════════════════════════════════════════════════════════════
