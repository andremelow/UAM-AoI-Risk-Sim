%% RERUN_MWPE  Roda mw-pe (Max-Weight Explicit Preemption) no grid (K,C) completo.
%
%  Usage (headless):
%    matlab -nodisplay -r "cd /mnt/stg/wrk/interlagos/simulator/v3/uam-sim-new; rerun_mwpe; exit"
...
logFile = fullfile(OUT_ROOT, sprintf('rerun_mwpe_%s.log', ts));
diary(logFile); diary on;

fprintf('\n');
fprintf('═══════════════════════════════════════════════════════════════\n');
fprintf('  RERUN mw-pe  (Max-Weight + Explicit Preemption, H_MAX=3)\n');
fprintf('  K: %s\n', num2str(K_values));
...
cfg_base.h_preempt = 3;   % h_s ceiling for forced C2 preemption
...
T_old = T_old(T_old.policy ~= "mw-pe", :);
...
fprintf('[RERUN_MWPE] Global summary updated → %s  (%d rows)\n', new_all, height(T_merged));
