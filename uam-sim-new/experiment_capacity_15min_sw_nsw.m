function experiment_capacity_15min_sw_nsw(varargin)
%% EXPERIMENT_CAPACITY_15MIN_SW_NSW  Sweep K × C × routing — switching vs no-switching.
%
%  Compara pares switching / no-switching das três políticas base:
...
%    Fase steady: C drones ativos por steady_duration = 900 s (15 min).
...
%    Policy  — round-robin | round-robin-sw | pf-classic | pf-classic-sw |
%              max-weight-drift | max-weight-sw
%
%  Total: 6K × 6C × 4 routing × 6 políticas = 864 runs.
%  Corredor: 2 km, dois lanes paralelos (North ±50 m), speed = 5 m/s.
%
%  Output: csv_export/capacity_15min_sw_nsw/K{K}/C{CC}/{routing_label}/
%
%  Usage:
%    experiment_capacity_15min_sw_nsw()
%    experiment_capacity_15min_sw_nsw('--smoke-only')
%    experiment_capacity_15min_sw_nsw('skip-smoke')
%    experiment_capacity_15min_sw_nsw('--yes')
%    experiment_capacity_15min_sw_nsw('--headless')
%    experiment_capacity_15min_sw_nsw('--C', '2,4,8')    % subset de capacidades
%    experiment_capacity_15min_sw_nsw('--K', '1,2,4')    % subset de K
...
OUT_ROOT = fullfile(SIM_ROOT, 'csv_export', 'capacity_15min_sw_nsw');
system(sprintf('mkdir -p "%s"', OUT_ROOT));

% ── Logfile ───────────────────────────────────────────────────────────
ts      = datestr(now, 'yyyymmdd_HHMMSS'); %#ok<TNOW1,DATST>
logFile = fullfile(OUT_ROOT, sprintf('experiment_capacity_15min_sw_nsw_%s.log', ts));
diary(logFile); diary on;

fprintf('\n');
fprintf('═══════════════════════════════════════════════════════════════\n');
fprintf('  EXPERIMENT_CAPACITY_15MIN_SW_NSW — switching vs no-switching\n');
...
if ~skip_smoke || smoke_only
    fprintf('\n──────────────────────────────────────────────────────────\n');
    fprintf('  SMOKE TEST (corredor 200m, C=2, K=1, T=60s, 4 políticas × 2 cenários)\n');
    fprintf('──────────────────────────────────────────────────────────\n\n');
    smoke_t0 = tic;
...
% ── Sweep ─────────────────────────────────────────────────────────────
fprintf('\n──────────────────────────────────────────────────────────\n');
fprintf('  SW_NSW SWEEP  (%d runs, %d workers)\n', n_combos, nWorkers);
fprintf('──────────────────────────────────────────────────────────\n\n');
...
fprintf('\n[SW_NSW] Sweep concluído em %.2f h\n', toc(sw_t0)/3600);
...
fprintf('[SW_NSW] Aggregated → %s\n', all_csv);

fprintf('\n═══════════════════════════════════════════════════════════════\n');
fprintf('  EXPERIMENT_CAPACITY_15MIN_SW_NSW COMPLETED\n');
fprintf('  Finished: %s\n', datestr(now)); %#ok<TNOW1,DATST>
...
fclose(fid);
fprintf('[SW_NSW] Summary → %s\n', filepath);
end


%% ═══════════════════════════════════════════════════════════════════
%%  Smoke test — corredor 200m, C=2, K=1, T=60s, 4 políticas × 2 cenários
%% ═══════════════════════════════════════════════════════════════════
